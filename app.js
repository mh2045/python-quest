...
async function apiLoad(uid) {
  try { var r=await fetch('/progress/'+encodeURIComponent(uid)); return await r.json(); }
  catch(e) { return {xp:0,completed:[]}; }
}
async function apiSave(uid,data) {
  try { await fetch('/progress/'+encodeURIComponent(uid),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}); }
  catch(e) {}
}
...
async function doLogin() {
  ...
  var data=await apiLoad(userId);
  xp=data.xp||0; completed=data.completed||[];
...
async function finishLesson() {
  ...
  if(!already){
    ...
    await apiSave(userId,{xp:xp,completed:completed});
  }
